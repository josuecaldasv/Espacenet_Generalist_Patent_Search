...
def search_patents_by_cpc(access_token, cpc_symbol, start=1, batch_size=100, max_results=1000):
...
def get_patent_biblio(doc_number, token):
    ...
    if response.status_code == 200:
        ...
        error_message = f"Error 404: No se encontró biblio para el documento {doc_number}"
        return error_message
    else:
        error_message = f"Error en la consulta de biblio ({response.status_code}) para el documento {doc_number}"
        return error_message
